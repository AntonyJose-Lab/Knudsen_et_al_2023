...
Setting k3 = 0 is used to examine the effect of not producing any secondary siRNAs and for elucidating the impact of varying mRNA turnover (Tm) on the extent of knockdown.
Author: Antony Jose
"""
...
os.makedirs('/Users/antonyjose/Desktop/JoseLab/Modeling/2022_2_9_onwards/RNA_silencing/analyses/Python/Figures/2022_7_29_RNAiDynamics', exist_ok=True)
...
min_m_arr = np.zeros(20)
Tm_arr = np.zeros(20)

for Tm in np.arange(0,1.0,0.05):
    ...
    k3 = 0       # 1 pUG RNA made per 1º siRNA-bound mRNAs.
    ...
    for i in range (1, n_steps + 1):
       ...
       if (min_m > m_arr[i]):
           min_m = m_arr[i]
    index = int(round(Tm/0.05))
    min_m_arr[index] = min_m
    Tm_arr[index] = Tm
    print(Tm_arr[index])
    ...
    plt.title('The dynamics of RNA interference in C. elegans', fontsize = 12)  # add some title to your plot
    ...
    plt.show()
    file_name = '2022_7_29_RNAiDynamics_arbitary_parameters_no_pUG_'
    path = os.path.join(curr_dir, 'Figures/2022_7_29_RNAiDynamics/', file_name + str(round(Tm,2)) + '.svg')
    fig.savefig(path, dpi=300)
# plot min_m vs Tm
fig, ax = plt.subplots()
plt.plot(Tm_arr, min_m_arr, linewidth = 1)
plt.title('Tm vs $[mRNA]_{min}$', fontsize = 12)  # add some title to your plot
plt.xlabel('Tm', fontsize = 8)
plt.ylabel('$[mRNA]_{min}$', fontsize = 8)
plt.xticks(fontsize = 8)
plt.yticks(fontsize = 8)
plt.grid(False)                 # do not show grid
ymax = max(min_m_arr) + 10
xmax = max(Tm_arr) + 0.05
plt.axis([0, xmax, 0, ymax])     # show axes measures
plt.show()
file_name = '2022_7_29_RNAiDynamics_arbitary_parameters_no_pUG_Tm_vs_min_m'
path = os.path.join(curr_dir, 'Figures/2022_7_29_RNAiDynamics/', file_name + '.svg')
fig.savefig(path, dpi=300)
