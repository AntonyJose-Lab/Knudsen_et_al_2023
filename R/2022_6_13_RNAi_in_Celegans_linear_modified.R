## This program aims to simulate the RNAi response for a somatic gene exposed to matching dsRNA in C. elegans in terms of readily measurable RNA species.
...
## run conditions for varying number of experiments to be considered. Comment out different conditions below and reassign variables accordingly to explore how each parameter contributes to outcomes.
run_condition <- "vary_pm_m_k1_to_k5"
...
pre_mRNA_increase_with_knockdown <- data.frame()
pre_mRNA_decrease_with_knockdown <- data.frame()
...
for (s in 1:10)
  ...
  for (i in 1:100000) {
    ...
    if ( m_i < m &&
       pm_i > 0 &&
       m_i > 0 ){ ## These conditions are minimal constraints on the constants k1 through k4
      parameters <- rbind(parameters, cbind(s, m, pm, k1, k2, k3, k4, k5, m_i, pm_i, m_p_ratio, m_i_pm_i_ratio, kd_i))
      if (m_i_pm_i_ratio < m_p_ratio) {
        pre_mRNA_increase_with_knockdown <- rbind(pre_mRNA_increase_with_knockdown, cbind(s, m, pm, k1, k2, k3, k4, k5, m_i, pm_i, m_p_ratio, m_i_pm_i_ratio, kd_i))
      }
      else {
        pre_mRNA_decrease_with_knockdown <- rbind(pre_mRNA_decrease_with_knockdown, cbind(s, m, pm, k1, k2, k3, k4, k5, m_i, pm_i, m_p_ratio, m_i_pm_i_ratio, kd_i))
      }
    }
    ...
    if ( m_il < m &&
        pm_il > 0 &&
        m_il > 0){ ## These conditions are minimal constraints on the constants k1 through k4
...
colnames(pre_mRNA_increase_with_knockdown) <- c("seed", "mRNA", "pre-mRNA", "k1","k2","k3","k4", "k5", "mRNA_after_RNAi", "pre-mRNA_after_RNAi", "mRNA/pre-mRNA_initial", "mRNA/pre-mRNA_after_RNAi", "knockdown")
write.table(pre_mRNA_increase_with_knockdown, file = paste0(path_to_table_outputs, "/RNAi_pre_mRNA_increases_", run_condition, ".csv"), sep = ",", row.names = FALSE)

colnames(pre_mRNA_decrease_with_knockdown) <- c("seed", "mRNA", "pre-mRNA", "k1","k2","k3","k4", "k5", "mRNA_after_RNAi", "pre-mRNA_after_RNAi", "mRNA/pre-mRNA_initial", "mRNA/pre-mRNA_after_RNAi", "knockdown")
write.table(pre_mRNA_decrease_with_knockdown, file = paste0(path_to_table_outputs, "/RNAi_pre_mRNA_decreases_", run_condition, ".csv"), sep = ",", row.names = FALSE)
...
## Select parameters that are leading to extremes of knockdown efficiency.
good_kd <- parameters %>% filter(knockdown < 0.1)
poor_kd <- parameters %>% filter(knockdown > 0.9)
...
######### fixed mRNA lengths and good knockdown
## plot pre-mRNA vs mRNA after RNAi as a function of initial mRNA concentration.
max_m_i = round(max(good_kd$`mRNA_after_RNAi`))
max_pm_i = round(max(good_kd$`pre-mRNA_after_RNAi`))
m_vs_m_i_vs_pm_i <- ggplot(good_kd, aes(x = `mRNA_after_RNAi`, y = `pre-mRNA_after_RNAi`, color = mRNA), alpha = 0.5) +
  my_theme + ylab("[pre-mRNA] after RNAi") + xlab("[mRNA] after RNAi") + geom_point(size = 2) +
  scale_color_gradient(low = "darkolivegreen1", high = "darkgreen") +
  labs(color = "initial [mRNA]") +
  scale_x_continuous(limits = c(0, max_m_i), breaks = c(0, 0.5*max_m_i, max_m_i), expand = c(0.05,0.05)) +
...
ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_initial_mRNA_vs_mRNA_and_premRNA_after_RNAi_good_kd_fixed_mRNA_length_", run_condition, ".eps"), m_vs_m_i_vs_pm_i, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot mRNA to pre-mRNA ratio before and after RNAi as it relates to levels of knockdown.
max_m_p_ratio = max(good_kd$`mRNA/pre-mRNA_initial`)
max_m_i_pm_i_ratio = max(good_kd$`mRNA/pre-mRNA_after_RNAi`)
m_vs_pm_vs_m_i_vs_pm_i <- ggplot(good_kd, aes(x = `mRNA/pre-mRNA_initial`, y = `mRNA/pre-mRNA_after_RNAi`, color = knockdown), alpha = 0.5) +
  my_theme + ylab("[mRNA]/[pre-mRNA] after RNAi") + xlab("[mRNA]/[pre-mRNA] initial") + geom_point(size = 2) +
...
ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_initial_mRNA_premRNA_vs_mRNA_premRNA_after_RNAi_good_kd_fixed_mRNA_length_", run_condition, ".eps"), m_vs_pm_vs_m_i_vs_pm_i, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot constants k1 through k5 in plausible parameter sets for mRNAs of fixed length.
plausible_constants <- good_kd[,4:8]
to_plot_constants <- melt(plausible_constants)
constants_jitter <- ggplot(to_plot_constants, aes(variable, value)) +
  my_theme + ylab("value") + xlab("constants") + geom_point(size = 2) + geom_jitter(position = "jitter")

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_plausible_constants_fixed_mRNA_length_", run_condition, ".eps"), constants_jitter, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot knockdown efficiency vs. pre-mRNA/mRNA ratio with fixed mRNA lengths.
max_m_i_pm_i_ratio = max(good_kd$`mRNA/pre-mRNA_after_RNAi`)
max_kd_i = max(good_kd$knockdown)
KD_vs_m_i_pm_i_ratio <- ggplot(good_kd, aes(x = `mRNA/pre-mRNA_after_RNAi`, y = knockdown), alpha = 0.5) +
  my_theme + ylab("(initial [mRNA])/([mRNA] after RNAi)") + xlab("[mRNA]/[pre-mRNA] after RNAi") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_m_i_pm_i_ratio), breaks = c(0, 0.5*max_m_i_pm_i_ratio, max_m_i_pm_i_ratio), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, 1), breaks= c(0, 0.5, 1), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_mRNA_pre-mRNA_ratio_vs_KD_fixed_mRNA_length_", run_condition, ".eps"), KD_vs_m_i_pm_i_ratio, height = 3, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot knockdown efficiency vs. amount of initial mRNA for fixed lengths of mRNA.
max_length = max(good_kd$mRNA)
max_kd_il = max(good_kd$knockdown)
KD_vs_m_i_pm_i_ratio <- ggplot(good_kd, aes(x = mRNA, y = knockdown), alpha = 0.5) +
  my_theme + ylab("(initial [mRNA])/([mRNA] after RNAi)") + xlab("[mRNA] before RNAi") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_length), breaks = c(0, 0.5*max_length, max_length), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, 1), breaks= c(0, 0.5, 1), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_mRNA_conc_vs_KD_fixed_mRNA_length_", run_condition, ".eps"), KD_vs_m_i_pm_i_ratio, height = 3, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

######### fixed mRNA lengths and poor knockdown
## plot pre-mRNA vs mRNA after RNAi as a function of initial mRNA concentration.
max_m_i = round(max(poor_kd$`mRNA_after_RNAi`))
max_pm_i = round(max(poor_kd$`pre-mRNA_after_RNAi`))
m_vs_m_i_vs_pm_i <- ggplot(poor_kd, aes(x = `mRNA_after_RNAi`, y = `pre-mRNA_after_RNAi`, color = mRNA), alpha = 0.5) +
  my_theme + ylab("[pre-mRNA] after RNAi") + xlab("[mRNA] after RNAi") + geom_point(size = 2) +
  scale_color_gradient(low = "darkolivegreen1", high = "darkgreen") +
  labs(color = "initial [mRNA]") +
  scale_x_continuous(limits = c(0, max_m_i), breaks = c(0, 0.5*max_m_i, max_m_i), expand = c(0.05,0.05)) +
...
ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_initial_mRNA_vs_mRNA_and_premRNA_after_RNAi_poor_kd_fixed_mRNA_length_", run_condition, ".eps"), m_vs_m_i_vs_pm_i, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot mRNA to pre-mRNA ratio before and after RNAi as it relates to levels of knockdown.
max_m_p_ratio = max(poor_kd$`mRNA/pre-mRNA_initial`)
max_m_i_pm_i_ratio = max(poor_kd$`mRNA/pre-mRNA_after_RNAi`)
m_vs_pm_vs_m_i_vs_pm_i <- ggplot(poor_kd, aes(x = `mRNA/pre-mRNA_initial`, y = `mRNA/pre-mRNA_after_RNAi`, color = knockdown), alpha = 0.5) +
  my_theme + ylab("[mRNA]/[pre-mRNA] after RNAi") + xlab("[mRNA]/[pre-mRNA] initial") + geom_point(size = 2) +
...
ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_initial_mRNA_premRNA_vs_mRNA_premRNA_after_RNAi_poor_kd_fixed_mRNA_length_", run_condition, ".eps"), m_vs_pm_vs_m_i_vs_pm_i, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot constants k1 through k5 in plausible parameter sets for mRNAs of fixed length.
plausible_constants <- poor_kd[,4:8]
to_plot_constants <- melt(plausible_constants)
constants_jitter <- ggplot(to_plot_constants, aes(variable, value)) +
  my_theme + ylab("value") + xlab("constants") + geom_point(size = 2) + geom_jitter(position = "jitter")

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_plausible_constants_fixed_mRNA_length_", run_condition, ".eps"), constants_jitter, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot knockdown efficiency vs. pre-mRNA/mRNA ratio with fixed mRNA lengths.
max_m_i_pm_i_ratio = max(poor_kd$`mRNA/pre-mRNA_after_RNAi`)
max_kd_i = max(poor_kd$knockdown)
KD_vs_m_i_pm_i_ratio <- ggplot(poor_kd, aes(x = `mRNA/pre-mRNA_after_RNAi`, y = knockdown), alpha = 0.5) +
  my_theme + ylab("(initial [mRNA])/([mRNA] after RNAi)") + xlab("[mRNA]/[pre-mRNA] after RNAi") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_m_i_pm_i_ratio), breaks = c(0, 0.5*max_m_i_pm_i_ratio, max_m_i_pm_i_ratio), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, 1), breaks= c(0, 0.5, 1), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_mRNA_pre-mRNA_ratio_vs_KD_fixed_mRNA_length_", run_condition, ".eps"), KD_vs_m_i_pm_i_ratio, height = 3, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot knockdown efficiency vs. amount of initial mRNA for fixed lengths of mRNA.
max_length = max(poor_kd$mRNA)
max_kd_il = max(poor_kd$knockdown)
KD_vs_m_i_pm_i_ratio <- ggplot(poor_kd, aes(x = mRNA, y = knockdown), alpha = 0.5) +
  my_theme + ylab("(initial [mRNA])/([mRNA] after RNAi)") + xlab("[mRNA] before RNAi") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_length), breaks = c(0, 0.5*max_length, max_length), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, 1), breaks= c(0, 0.5, 1), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_mRNA_conc_vs_KD_fixed_mRNA_length_", run_condition, ".eps"), KD_vs_m_i_pm_i_ratio, height = 3, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)
...
## plot mRNA to pre-mRNA ratio before and after RNAi as it relates to levels of knockdown and that all have increased pre-mRNA levels.
max_m_p_ratio = max(pre_mRNA_increase_with_knockdown$`mRNA/pre-mRNA_initial`)
max_m_i_pm_i_ratio = max(pre_mRNA_increase_with_knockdown$`mRNA/pre-mRNA_after_RNAi`)
m_vs_pm_vs_m_i_vs_pm_i <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = `mRNA/pre-mRNA_initial`, y = `mRNA/pre-mRNA_after_RNAi`, color = knockdown), alpha = 0.5) +
  my_theme + ylab("[mRNA]/[pre-mRNA] after RNAi") + xlab("[mRNA]/[pre-mRNA] initial") + geom_point(size = 2) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(color = "knock down") +
  scale_x_continuous(limits = c(0, 10), breaks = c(0, 0.5*10, 10), expand = c(0.05,0.05)) + ## Note the axis limits here have been hard coded to see trends better.
...
ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_initial_mRNA_premRNA_vs_mRNA_premRNA_after_RNAi_fixed_mRNA_length_", run_condition, ".eps"), m_vs_pm_vs_m_i_vs_pm_i, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

## plot mRNA to pre-mRNA ratio before and after RNAi as it relates to levels of knockdown and that all have decreased pre-mRNA levels.
max_m_p_ratio = max(pre_mRNA_decrease_with_knockdown$`mRNA/pre-mRNA_initial`)
max_m_i_pm_i_ratio = max(pre_mRNA_decrease_with_knockdown$`mRNA/pre-mRNA_after_RNAi`)
m_vs_pm_vs_m_i_vs_pm_i <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = `mRNA/pre-mRNA_initial`, y = `mRNA/pre-mRNA_after_RNAi`, color = knockdown), alpha = 0.5) +
  my_theme + ylab("[mRNA]/[pre-mRNA] after RNAi") + xlab("[mRNA]/[pre-mRNA] initial") + geom_point(size = 2) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(color = "knock down") +
  scale_x_continuous(limits = c(0, 10), breaks = c(0, 0.5*10, 10), expand = c(0.05,0.05)) + ## Note the axis limits here have been hard coded to see trends better.
...
ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_initial_mRNA_premRNA_vs_mRNA_premRNA_after_RNAi_fixed_mRNA_length_", run_condition, ".eps"), m_vs_pm_vs_m_i_vs_pm_i, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)
...
### Plot each constant against all other constants with fixed length and good knockdown.
### k1 vs k2, k1 vs k3, k1 vs k4, k1 vs k5, k2 vs k3, k2 vs k4, k2 vs k5, k3 vs k4, k3 vs k5, and k4 vs k5
# k1 vs k2
max_k1 = round(max(good_kd$k1), 1)
max_k2 = round(max(good_kd$k2), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k1, y = k2), alpha = 0.5) +
  my_theme + ylab("k2") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k2), breaks= c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k1_vs_k2_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k3
max_k1 = round(max(good_kd$k1), 1)
max_k3 = round(max(good_kd$k3), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k1, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k1_vs_k3_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k4
max_k1 = round(max(good_kd$k1), 1)
max_k4 = round(max(good_kd$k4), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k1, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k1_vs_k4_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k5
max_k1 = round(max(good_kd$k1), 1)
max_k5 = round(max(good_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k1, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k1_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k3
max_k2 = round(max(good_kd$k2), 1)
max_k3 = round(max(good_kd$k3), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k2, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k2_vs_k3_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k4
max_k2 = round(max(good_kd$k2), 1)
max_k4 = round(max(good_kd$k4), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k2, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k2_vs_k4_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k5
max_k2 = round(max(good_kd$k2), 1)
max_k5 = round(max(good_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k2, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k2_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k4
max_k3 = round(max(good_kd$k3), 1)
max_k4 = round(max(good_kd$k4), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k3, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k3_vs_k4_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k5
max_k3 = round(max(good_kd$k3), 1)
max_k5 = round(max(good_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k3, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k3_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k4 vs k5
max_k4 = round(max(good_kd$k4), 1)
max_k5 = round(max(good_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(good_kd, aes(x = k4, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k4") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k4), breaks = c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_good_kd_constants_vs_constants_fixed_gene_length_k4_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

### Plot each constant against all other constants with fixed length and poor knockdown.
### k1 vs k2, k1 vs k3, k1 vs k4, k1 vs k5, k2 vs k3, k2 vs k4, k2 vs k5, k3 vs k4, k3 vs k5, and k4 vs k5
# k1 vs k2
max_k1 = round(max(poor_kd$k1), 1)
max_k2 = round(max(poor_kd$k2), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k1, y = k2), alpha = 0.5) +
  my_theme + ylab("k2") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k2), breaks= c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k1_vs_k2_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k3
max_k1 = round(max(poor_kd$k1), 1)
max_k3 = round(max(poor_kd$k3), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k1, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k1_vs_k3_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k4
max_k1 = round(max(poor_kd$k1), 1)
max_k4 = round(max(poor_kd$k4), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k1, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k1_vs_k4_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k5
max_k1 = round(max(poor_kd$k1), 1)
max_k5 = round(max(poor_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k1, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k1_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k3
max_k2 = round(max(poor_kd$k2), 1)
max_k3 = round(max(poor_kd$k3), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k2, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k2_vs_k3_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k4
max_k2 = round(max(poor_kd$k2), 1)
max_k4 = round(max(poor_kd$k4), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k2, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k2_vs_k4_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k5
max_k2 = round(max(poor_kd$k2), 1)
max_k5 = round(max(poor_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k2, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k2_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k4
max_k3 = round(max(poor_kd$k3), 1)
max_k4 = round(max(poor_kd$k4), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k3, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k3_vs_k4_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k5
max_k3 = round(max(poor_kd$k3), 1)
max_k5 = round(max(poor_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k3, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k3_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k4 vs k5
max_k4 = round(max(poor_kd$k4), 1)
max_k5 = round(max(poor_kd$k5), 1)
constants_vs_constants_fixed <- ggplot(poor_kd, aes(x = k4, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k4") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k4), breaks = c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_poor_kd_constants_vs_constants_fixed_gene_length_k4_vs_k5_", run_condition, ".eps"), constants_vs_constants_fixed, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)
...
### Plot each constant against all other constants with varying length with increases in relative pre-mRNA after knockdown.
### k1 vs k2, k1 vs k3, k1 vs k4, k1 vs k5, k2 vs k3, k2 vs k4, k2 vs k5, k3 vs k4, k3 vs k5, and k4 vs k5
# k1 vs k2
max_k1 = round(max(pre_mRNA_increase_with_knockdown$k1), 1)
max_k2 = round(max(pre_mRNA_increase_with_knockdown$k2), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k1, y = k2), alpha = 0.5) +
  my_theme + ylab("k2") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k2), breaks= c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k1_vs_k2_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k3
max_k1 = round(max(pre_mRNA_increase_with_knockdown$k1), 1)
max_k3 = round(max(pre_mRNA_increase_with_knockdown$k3), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k1, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k1_vs_k3_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k4
max_k1 = round(max(pre_mRNA_increase_with_knockdown$k1), 1)
max_k4 = round(max(pre_mRNA_increase_with_knockdown$k4), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k1, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k1_vs_k4_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k5
max_k1 = round(max(pre_mRNA_increase_with_knockdown$k1), 1)
max_k5 = round(max(pre_mRNA_increase_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k1, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k1_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k3
max_k2 = round(max(pre_mRNA_increase_with_knockdown$k2), 1)
max_k3 = round(max(pre_mRNA_increase_with_knockdown$k3), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k2, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k2_vs_k3_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k4
max_k2 = round(max(pre_mRNA_increase_with_knockdown$k2), 1)
max_k4 = round(max(pre_mRNA_increase_with_knockdown$k4), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k2, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k2_vs_k4_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k5
max_k2 = round(max(pre_mRNA_increase_with_knockdown$k2), 1)
max_k5 = round(max(pre_mRNA_increase_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k2, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k2_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k4
max_k3 = round(max(pre_mRNA_increase_with_knockdown$k3), 1)
max_k4 = round(max(pre_mRNA_increase_with_knockdown$k4), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k3, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k3_vs_k4_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k5
max_k3 = round(max(pre_mRNA_increase_with_knockdown$k3), 1)
max_k5 = round(max(pre_mRNA_increase_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k3, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k3_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k4 vs k5
max_k4 = round(max(pre_mRNA_increase_with_knockdown$k4), 1)
max_k5 = round(max(pre_mRNA_increase_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_increase_with_knockdown, aes(x = k4, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k4") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k4), breaks = c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_increases_constants_vs_constants_varying_gene_length_k4_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)


### Plot each constant against all other constants with varying length with decreases in relative pre-mRNA after knockdown.
### k1 vs k2, k1 vs k3, k1 vs k4, k1 vs k5, k2 vs k3, k2 vs k4, k2 vs k5, k3 vs k4, k3 vs k5, and k4 vs k5
# k1 vs k2
max_k1 = round(max(pre_mRNA_decrease_with_knockdown$k1), 1)
max_k2 = round(max(pre_mRNA_decrease_with_knockdown$k2), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k1, y = k2), alpha = 0.5) +
  my_theme + ylab("k2") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k2), breaks= c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k1_vs_k2_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k3
max_k1 = round(max(pre_mRNA_decrease_with_knockdown$k1), 1)
max_k3 = round(max(pre_mRNA_decrease_with_knockdown$k3), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k1, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k1_vs_k3_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k4
max_k1 = round(max(pre_mRNA_decrease_with_knockdown$k1), 1)
max_k4 = round(max(pre_mRNA_decrease_with_knockdown$k4), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k1, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k1_vs_k4_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k1 vs k5
max_k1 = round(max(pre_mRNA_decrease_with_knockdown$k1), 1)
max_k5 = round(max(pre_mRNA_decrease_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k1, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k1") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k1), breaks = c(0, 0.5*max_k1, max_k1), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k1_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k3
max_k2 = round(max(pre_mRNA_decrease_with_knockdown$k2), 1)
max_k3 = round(max(pre_mRNA_decrease_with_knockdown$k3), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k2, y = k3), alpha = 0.5) +
  my_theme + ylab("k3") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k3), breaks= c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k2_vs_k3_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k4
max_k2 = round(max(pre_mRNA_decrease_with_knockdown$k2), 1)
max_k4 = round(max(pre_mRNA_decrease_with_knockdown$k4), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k2, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k2_vs_k4_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k2 vs k5
max_k2 = round(max(pre_mRNA_decrease_with_knockdown$k2), 1)
max_k5 = round(max(pre_mRNA_decrease_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k2, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k2") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k2), breaks = c(0, 0.5*max_k2, max_k2), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k2_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k4
max_k3 = round(max(pre_mRNA_decrease_with_knockdown$k3), 1)
max_k4 = round(max(pre_mRNA_decrease_with_knockdown$k4), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k3, y = k4), alpha = 0.5) +
  my_theme + ylab("k4") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k4), breaks= c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k3_vs_k4_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k3 vs k5
max_k3 = round(max(pre_mRNA_decrease_with_knockdown$k3), 1)
max_k5 = round(max(pre_mRNA_decrease_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k3, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k3") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k3), breaks = c(0, 0.5*max_k3, max_k3), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k3_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)

# k4 vs k5
max_k4 = round(max(pre_mRNA_decrease_with_knockdown$k4), 1)
max_k5 = round(max(pre_mRNA_decrease_with_knockdown$k5), 1)
constants_vs_constants_varying <- ggplot(pre_mRNA_decrease_with_knockdown, aes(x = k4, y = k5), alpha = 0.5) +
  my_theme + ylab("k5") + xlab("k4") + geom_point(size = 2) +
  scale_x_continuous(limits = c(0, max_k4), breaks = c(0, 0.5*max_k4, max_k4), expand = c(0.05,0.05)) +
  scale_y_continuous(limits = c(0, max_k5), breaks= c(0, 0.5*max_k5, max_k5), expand = c(0.05,0.05))

ggsave(file = paste0(path_to_figure_outputs, "/RNAi_premRNA_decreases_constants_vs_constants_varying_gene_length_k4_vs_k5_", run_condition, ".eps"), constants_vs_constants_varying, height = 2.5, width = 3.5, device = "eps", units = "in", dpi = 300, limitsize = FALSE)
